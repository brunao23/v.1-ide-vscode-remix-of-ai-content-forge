...
const email=`rag-nullcontent-${Date.now()}@gmail.com`;
...
const docRes=await req('POST',`${URL}/rest/v1/documents`,{apikey:SRK,authorization:`Bearer ${SRK}`,Prefer:'return=representation'},[{user_id:userId,tenant_id:tenantId,name:`qa-nullcontent-${Date.now()}`,type:'brand-book',agent_id:'brand-book',content:null,processing_status:'ready'}]);
if(!docRes.ok) throw new Error(JSON.stringify(docRes));
const docId=docRes.data[0].id;

const chunkRes=await req('POST',`${URL}/rest/v1/document_chunks`,{apikey:SRK,authorization:`Bearer ${SRK}`,Prefer:'return=representation'},[{document_id:docId,user_id:userId,tenant_id:tenantId,chunk_index:0,content:'Trecho do brand book armazenado em chunks mesmo sem content no documento.',tokens:20}]);
if(!chunkRes.ok) throw new Error(JSON.stringify(chunkRes));
...
console.log(JSON.stringify({ok:chat.ok,status:chat.status,docs:chat.ok?chat.data?.documentsContext:null,preview:chat.ok?String(chat.data?.content||'').slice(0,220):null,error:chat.ok?null:chat.data},null,2));
