...
const email=`rag-alias-${Date.now()}@gmail.com`;
...
// Simula legado: type fora do canonico, mas com agent_id de brand-book
const ins=await req('POST',`${URL}/rest/v1/documents`,{apikey:SRK,authorization:`Bearer ${SRK}`,Prefer:'return=representation'},[{user_id:userId,tenant_id:tenantId,name:`qa-alias-${Date.now()}`,type:'outro',agent_id:'brand-book',content:'Brand Book legado com type outro mas agent_id correto.',processing_status:'ready'}]);
...
console.log(JSON.stringify({ok:chat.ok,status:chat.status,docs:chat.ok?chat.data?.documentsContext:null,contentPreview:chat.ok?String(chat.data?.content||'').slice(0,220):null,error:chat.ok?null:chat.data},null,2));
