...
export function EmailListChart({ history }: Props) {
  const data = history.map((h) => ({
    label: `${MONTHS[h.period_month - 1]}/${String(h.period_year).slice(2)}`,
    value: h.email_list_size || 0,
  }));

  if (!data.length) {
    return (
      <div className="rounded-[10px] bg-secondary border border-border p-6">
        <h3 className="text-sm font-semibold text-muted-foreground mb-4">Inscritos Lista de E-mail</h3>
  ...
  return (
    <div className="rounded-[10px] bg-secondary border border-border p-6">
      <h3 className="text-sm font-semibold text-muted-foreground mb-4">Inscritos Lista de E-mail (últimos meses)</h3>
